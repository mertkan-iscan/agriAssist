...
@Slf4j
@Service
public class UdpDeviceJoinService {
    ...
    @Autowired
    public UdpDeviceJoinService(DeviceService deviceService, FieldService fieldService, ScheduledSensorDataFetcher scheduledSensorDataFetcher, SimpMessagingTemplate messagingTemplate, DeviceCommandConfigLoader deviceCommandConfigLoader) {
    ...
    public void startJoinServer(int port) {
        try (DatagramSocket socket = new DatagramSocket(port)) {
            ...
            while (true) {
                ...
                clientHandlerPool.submit(() -> handleDeviceJoinRequest(packet, socket));
            }
        } catch (IOException e) {
    ...
    private void handleDeviceJoinRequest(DatagramPacket packet, DatagramSocket socket) {
        try {
            String joinRequest = new String(packet.getData(), 0, packet.getLength());
            log.info("Received join request from {}: {}", packet.getAddress(), joinRequest);

            processJoinRequest(joinRequest, packet.getAddress(), packet.getPort(), socket);
        } catch (Exception e) {
            log.error("Error while handling device join request.", e);
        }
    }

    private void processJoinRequest(String joinRequest, InetAddress deviceAddress, int devicePort, DatagramSocket socket) {
        try {
            ...
            Device device = new Device(deviceID, null, deviceModel, deviceAddress.getHostAddress(), deviceType);
            device.setDevicePort(devicePort);

            DeviceSocketWrapper deviceWrapper = new DeviceSocketWrapper(null, device);
            ...
            if (existingDevice == null) {
                ...
                sendDeviceJoinAcceptResponse(deviceAddress, devicePort, socket);
    ...
    public String acceptDevice(int deviceID, int fieldID) {
        ...
        try {
            ...
            sendDeviceJoinAcceptResponse(InetAddress.getByName(device.getDeviceIp()), device.getDevicePort(), new DatagramSocket());
    ...
    public String refuseDevice(int deviceID) {
        ...
        try {
            sendDeviceJoinRefuseResponse(InetAddress.getByName(deviceWrapper.getDeviceObj().getDeviceIp()), deviceWrapper.getDeviceObj().getDevicePort(), new DatagramSocket());
    ...
    public void sendDeviceJoinAcceptResponse(InetAddress deviceAddress, int devicePort, DatagramSocket socket) {
        try {
            String message = deviceCommandConfigLoader.getDeviceJoinResponse("join_accepted");
            byte[] buffer = message.getBytes();

            DatagramPacket packet = new DatagramPacket(buffer, buffer.length, deviceAddress, devicePort);
            socket.send(packet);
    ...
    public void sendDeviceJoinRefuseResponse(InetAddress deviceAddress, int devicePort, DatagramSocket socket) {
        try {
